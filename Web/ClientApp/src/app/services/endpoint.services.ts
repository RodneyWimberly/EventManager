...
import { Subject, from, throwError, forkJoin } from 'rxjs';
...
export class EventEndpointService extends BaseEndpointService {
    ...
    getAllEventLocationsByEventsPaged(eventIds: number[], pageNumber: number, pageSize: number, includePropertyPaths: string | null): Observable<EventLocation[]> {
      var locations: Observable<EventLocation[]>;

      locations = this.getAllEventLocationsPaged(pageNumber, pageSize, "event");
      eventIds.forEach((value: number, index: number, list: number[]) => { forkJoin(locations, this.getAllEventLocationsByEventPaged(value, pageNumber, pageSize, includePropertyPaths)); })
      return locations;
    }
...
/** 1 = Sunday 2 = Monday 4 = Tuesday 8 = Wednesday 16 = Thursday 32 = Friday 64 = Saturday 127 = AllDays */
