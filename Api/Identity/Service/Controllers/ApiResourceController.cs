...
namespace EventManager.Identity.Service.Controllers
{
    ...
    public class ApiResourcesController : ApiController
    {
        ...
        public Task<ActionResult<IEnumerable<ApiScope>>> Scopes(string resource)
        {
            IEnumerable<ApiScope> clients = new List<ApiScope>();//  await _apiResourceAppService.GetScopes(resource);
            return Task.FromResult(ResponseGet(clients));
        }
        ...
        public async Task<ActionResult<IEnumerable<ApiScope>>> SaveScope(string resource, [FromBody] SaveApiScopeViewModel model)
        {
            if (!ModelState.IsValid)
            {
                NotifyModelStateErrors();
            ...
            IEnumerable<ApiScope> scopes = new List<ApiScope>();// await _apiResourceAppService.GetScopes(resource);
