...
namespace EventManager.Identity.DataAccess.Fido
{
    public class Fido2Storage
    {
        ...
        {
            var items = await _identityDbContext.FidoStoredCredential.Where(c => c.Username == username).ToListAsync();
            if (items != null)
            {
                foreach (var fido2Key in items)
                {
                    _identityDbContext.FidoStoredCredential.Remove(fido2Key);
                };
