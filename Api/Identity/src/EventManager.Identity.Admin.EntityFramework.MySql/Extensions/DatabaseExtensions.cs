...
namespace EventManager.Identity.Admin.EntityFramework.MySql.Extensions
{
            ...
            where TDataProtectionDbContext : DbContext, IDataProtectionKeyContext
        {
            ...
            services.AddConfigurationDbContext<TConfigurationDbContext>(options =>
                options.ConfigureDbContext = b =>
                    b.UseMySql(configurationConnectionString, ServerVersion.AutoDetect(configurationConnectionString), sql => sql.MigrationsAssembly(migrationsAssembly)));

            // Operational DB from existing connection
            services.AddOperationalDbContext<TPersistedGrantDbContext>(options => options.ConfigureDbContext = b =>
                b.UseMySql(persistedGrantConnectionString, ServerVersion.AutoDetect(persistedGrantConnectionString), sql => sql.MigrationsAssembly(migrationsAssembly)));

            // Log DB from existing connection
            services.AddDbContext<TLogDbContext>(options => options.UseMySql(errorLoggingConnectionString, ServerVersion.AutoDetect(errorLoggingConnectionString),
                optionsSql => optionsSql.MigrationsAssembly(migrationsAssembly)));

            // Audit logging connection
            services.AddDbContext<TAuditLoggingDbContext>(options => options.UseMySql(auditLoggingConnectionString, ServerVersion.AutoDetect(auditLoggingConnectionString),
                optionsSql => optionsSql.MigrationsAssembly(migrationsAssembly)));
